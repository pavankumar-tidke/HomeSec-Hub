...
async def set_arm_state(arm: bool):
    await db[config.ARM_STATUS_COLLECTION].update_one(
        {}, {"$set": {"status": arm, "updatedAt": datetime.utcnow()}}, upsert=True
    )
    return {"status": arm}

async def get_arm_state():
    doc = await db[config.ARM_STATUS_COLLECTION].find_one({})
...
class ArmService:
    ...
    async def _enable_sensors(self) -> Dict[str, Any]:
        ...
        try:
            # Mock sensor enablement - in real implementation, this would communicate with actual sensors
